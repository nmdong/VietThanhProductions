...
public final class DropDown: UIView {
	...
	*/
	...
	//MARK: Constraints
	fileprivate var heightConstraintNew: NSLayoutConstraint!
	fileprivate var widthConstraintNew: NSLayoutConstraint!
...
extension DropDown {
	...
	public override func updateConstraints() {
		...
		widthConstraintNew.constant = layout.width
		heightConstraintNew.constant = layout.visibleHeight
	...
	fileprivate func setupConstraints() {
		...
		widthConstraintNew = NSLayoutConstraint(
			item: tableViewContainer,
			attribute: .width,
			relatedBy: .equal,
			toItem: nil,
			...
			multiplier: 1,
			constant: 0)
		tableViewContainer.addConstraint(widthConstraintNew)

		heightConstraintNew = NSLayoutConstraint(
			item: tableViewContainer,
			attribute: .height,
			relatedBy: .equal,
			toItem: nil,
			...
			multiplier: 1,
			constant: 0)
		tableViewContainer.addConstraint(heightConstraintNew)
...
//		tableViewContainer.layer.shadowPath = shadowPath.cgPath
	...
	fileprivate func computeLayoutBottomDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let width = SCREEN_WIDTH - bottomOffset.x
	...
	fileprivate func computeLayoutForTopDisplay(window: UIWindow) -> ComputeLayoutTuple {
		...
		let width = SCREEN_WIDTH - topOffset.x
